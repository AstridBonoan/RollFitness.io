...
async function uploadPlanMediaFile(
  slug: string,
  kind: "photo" | "video",
  file: File,
) {
  ...
  const path = buildPlanMediaPath(slug, kind, file.type);
  if (!path) {
    return { error: "Unsupported file type." };
  }

  const supabase = await createClient();
  const folder = `plans/${slug}`;
...
export async function uploadPlanPhoto(slug: string, file: File) {
  return uploadPlanMediaFile(slug, "photo", file);
}

export async function uploadPlanVideo(slug: string, file: File) {
  return uploadPlanMediaFile(slug, "video", file);
}

export async function clearPlanMedia(slug: string, kind: "photo" | "video") {
  if (!(await isCurrentUserAdmin())) {
    return { error: "Admin access required." };
  }
